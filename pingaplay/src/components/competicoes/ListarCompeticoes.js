...
const RenderizarCompeticoes = ({ competitions }) => {
    return competitions.map((competition, index) => (
        <tr key={competition.id || index}>
            ...
            <td>{competition.título}</td>
            <td>{competition.descrição}</td>
            <td>{competition.local}</td>
            <td>{competition.data.toLocaleDateString()}</td>
        </tr>
    ));
};
    ...
    return <div>
      ...
      <div>
        <table className="table table-striped">
          ...
          { <tbody>
            <RenderizarCompeticoes competitions={competitions} /> {/* Chamando como componente */}
          </tbody> }
        </table>
      </div>
    </div>;
